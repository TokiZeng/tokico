...
if test -z "$tokicod"
then
  echo "tokicod not found"
  exit 1
fi
echo "Found: $tokicod"

TORDIR="$DIR/tokico-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting tokicod"
  tail -n 400 "$HOME/.bittokico/bittokico.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Tokico\ \'" | tail -n 20
  exit 1
fi
