...
gsigs = 'https://github.com/tokico-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('tokico'):
        # Remove the potentially stale tokico dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'tokico', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'tokico'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/tokico/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'tokico='+args.commit, '--url', 'tokico='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/tokico-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/tokico/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/tokico/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/tokico-project/tokico', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/tokico', exist_ok=True)
    os.chdir('builder/inputs/tokico')
    if args.pull:
