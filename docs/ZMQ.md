# The Current/Future Status of ZMQ in Tokico

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Tokico
...
matching supported by both Tokico and ZMQ. The `format`, `context` and `event`
...
The Tokico daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Tokico daemon also ensures that
...
The Tokico daemon will send a `txpool_add` pub exactly once for each
