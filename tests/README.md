...
Core tests take longer than any other Tokico tests, due to the high amount of computational work involved in validating core components.
...
To run only Tokico's core tests (after building):
...
To run only Tokico's crypto tests (after building):
...
```bash
tokicod --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `tokico-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Tokico's hash tests (after building):
...
To run only Tokico's performance tests (after building):
...
To run only Tokico's unit tests (after building):
