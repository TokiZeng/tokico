...
# a main offline tokicod, does most of the tests
# a restricted RPC tokicod setup with RPC payment
# two local online tokicods connected to each other
N_TOKICODS = 4

# 4 wallets connected to the main offline tokicod
# 1 wallet connected to the first local online tokicod
# 1 offline wallet
N_WALLETS = 6
...
tokicod_base = [builddir + "/bin/tokicod", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "tokicod_p2p_port", "--rpc-bind-port", "tokicod_rpc_port", "--zmq-rpc-bind-port", "tokicod_zmq_port", "--zmq-pub", "tokicod_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "tokicod_data_dir", "--log-level", "1"]
tokicod_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/tokico-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_TOKICODS):
  command_lines.append([str(18180+i) if x == "tokicod_rpc_port" else str(18280+i) if x == "tokicod_p2p_port" else str(18380+i) if x == "tokicod_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "tokicod_zmq_pub" else builddir + "/functional-tests-directory/tokicod" + str(i) if x == "tokicod_data_dir" else x for x in tokicod_base])
  if i < len(tokicod_extra):
    command_lines[-1] += tokicod_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/tokicod' + str(i) + '.log', 'a+'))
